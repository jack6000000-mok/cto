...
static const char *kBackend = "ssh-keygen";
...
KeyManager::KeyManager(QWidget *parent) : QMainWindow(parent) {
    ...
    lay->addWidget(title);
auto *warn = new QLabel("READ-ONLY. Displays public fingerprints only. Private keys never leave disk.");
...
lay->addWidget(warn);
    ...
    m_args = new QLineEdit("-l -f ~/.ssh/cto_github_rsa.pub");
    m_args->setPlaceholderText("args");
    m_btnRun = new QPushButton("▶ Run");
    m_btnStop = new QPushButton("■ Stop");
    m_btnStop->setEnabled(false);
    m_btnAbout = new QPushButton("ⓘ About");
    connect(m_btnRun, &QPushButton::clicked, this, &KeyManager::runTool);
    connect(m_btnStop, &QPushButton::clicked, this, &KeyManager::stopTool);
    connect(m_btnAbout, &QPushButton::clicked, this, &KeyManager::showAbout);
...
void KeyManager::runTool() {
    ...
    connect(m_proc, &QProcess::readyReadStandardOutput, this, &KeyManager::onOut);
    connect(m_proc, &QProcess::readyReadStandardError, this, &KeyManager::onErr);
    connect(m_proc, &QProcess::finished, this, &KeyManager::onDone);
    m_proc->start(kBackend, args);
    m_btnRun->setEnabled(false); m_btnStop->setEnabled(true);
}

void KeyManager::stopTool() {
    if (m_proc) { m_log->append("[stopping…]"); m_proc->kill(); }
}

void KeyManager::onOut() { m_log->append(QString::fromLocal8Bit(m_proc->readAllStandardOutput()).trimmed()); }
void KeyManager::onErr() { m_log->append(QString::fromLocal8Bit(m_proc->readAllStandardError()).trimmed()); }

void KeyManager::onDone(int code) {
