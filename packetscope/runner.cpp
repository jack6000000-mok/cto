...
PacketScope::PacketScope(QWidget *parent) : QMainWindow(parent) {
    ...
    setStyleSheet("QMainWindow{background:#0a0e14;} QWidget{background:#0a0e14;color:#c8e6c9;}"
                  ...
                  "QLineEdit{border:1px solid #00ffee;padding:8px;background:#111927;}"
                  "QTextEdit{background:#05070b;color:#00ffee;font-family:monospace;}");
    ...
    lay->addWidget(title);
auto *warn = new QLabel("Needs root/cap_net_raw for live capture.");
...
lay->addWidget(warn);
    ...
    m_args = new QLineEdit("-i any -c 100");
    m_args->setPlaceholderText("args");
    m_btnRun = new QPushButton("▶ Run");
    m_btnStop = new QPushButton("■ Stop");
    m_btnStop->setEnabled(false);
    m_btnAbout = new QPushButton("ⓘ About");
    connect(m_btnRun, &QPushButton::clicked, this, &PacketScope::runTool);
    connect(m_btnStop, &QPushButton::clicked, this, &PacketScope::stopTool);
    connect(m_btnAbout, &QPushButton::clicked, this, &PacketScope::showAbout);
    ...
    m_log->append(QString("PacketScope v0.1.0 — proprietary GUI. Backend: %1 (theirs).").arg(kBackend));
    lay->addWidget(m_log, 1);
}

void PacketScope::runTool() {
    if (m_proc) return;
    ...
    connect(m_proc, &QProcess::readyReadStandardOutput, this, &PacketScope::onOut);
    connect(m_proc, &QProcess::readyReadStandardError, this, &PacketScope::onErr);
    connect(m_proc, &QProcess::finished, this, &PacketScope::onDone);
    m_proc->start(kBackend, args);
    m_btnRun->setEnabled(false); m_btnStop->setEnabled(true);
}

void PacketScope::stopTool() {
    if (m_proc) { m_log->append("[stopping…]"); m_proc->kill(); }
}

void PacketScope::onOut() { m_log->append(QString::fromLocal8Bit(m_proc->readAllStandardOutput()).trimmed()); }
void PacketScope::onErr() { m_log->append(QString::fromLocal8Bit(m_proc->readAllStandardError()).trimmed()); }

void PacketScope::onDone(int code) {
...
void PacketScope::showAbout() {
    QMessageBox::about(this, "About PacketScope",
        "PacketScope v0.1.0 — © 2026 oday, All Rights Reserved (Proprietary).\n\n"
