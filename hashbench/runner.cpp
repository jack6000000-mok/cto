...
static const char *kBackend = "hashcat";
...
HashBench::HashBench(QWidget *parent) : QMainWindow(parent) {
    setWindowTitle("HashBench [Proprietary] — v0.2.1");
    resize(820, 560);
    ...
    lay->addWidget(title);
auto *warn = new QLabel("Audit only hashes you own or are authorized to test.");
...
lay->addWidget(warn);
    ...
    m_args = new QLineEdit("-b");
    m_args->setPlaceholderText("args");
    m_btnRun = new QPushButton("▶ Run");
    m_btnStop = new QPushButton("■ Stop");
    m_btnStop->setEnabled(false);
    m_btnAbout = new QPushButton("ⓘ About");
    connect(m_btnRun, &QPushButton::clicked, this, &HashBench::runTool);
    connect(m_btnStop, &QPushButton::clicked, this, &HashBench::stopTool);
    connect(m_btnAbout, &QPushButton::clicked, this, &HashBench::showAbout);
    ...
    m_log->append(QString("HashBench v0.2.1 — proprietary GUI. Backend: %1 (theirs).").arg(kBackend));
    lay->addWidget(m_log, 1);
}

void HashBench::runTool() {
    if (m_proc) return;
    ...
    connect(m_proc, &QProcess::readyReadStandardOutput, this, &HashBench::onOut);
    connect(m_proc, &QProcess::readyReadStandardError, this, &HashBench::onErr);
    connect(m_proc, &QProcess::finished, this, &HashBench::onDone);
    m_proc->start(kBackend, args);
    m_btnRun->setEnabled(false); m_btnStop->setEnabled(true);
}

void HashBench::stopTool() {
    if (m_proc) { m_log->append("[stopping…]"); m_proc->kill(); }
}

void HashBench::onOut() { m_log->append(QString::fromLocal8Bit(m_proc->readAllStandardOutput()).trimmed()); }
void HashBench::onErr() { m_log->append(QString::fromLocal8Bit(m_proc->readAllStandardError()).trimmed()); }

void HashBench::onDone(int code) {
...
void HashBench::showAbout() {
    QMessageBox::about(this, "About HashBench",
        "HashBench v0.2.1 — © 2026 oday, All Rights Reserved (Proprietary).\n\n"
