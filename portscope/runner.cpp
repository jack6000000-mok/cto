...
PortScope::PortScope(QWidget *parent) : QMainWindow(parent) {
    ...
    m_target = new QLineEdit("127.0.0.1");
    m_target->setPlaceholderText("target");
    m_args = new QLineEdit("-sV -sC -Pn");
    m_args->setPlaceholderText("args");
    m_btnRun = new QPushButton("▶ Run");
    m_btnStop = new QPushButton("■ Stop");
    m_btnStop->setEnabled(false);
    m_btnAbout = new QPushButton("ⓘ About");
    connect(m_btnRun, &QPushButton::clicked, this, &PortScope::runTool);
    connect(m_btnStop, &QPushButton::clicked, this, &PortScope::stopTool);
    connect(m_btnAbout, &QPushButton::clicked, this, &PortScope::showAbout);
    ...
    m_log->append(QString("PortScope v0.1.0 — proprietary GUI. Backend: %1 (theirs).").arg(kBackend));
    lay->addWidget(m_log, 1);
}

void PortScope::runTool() {
    if (m_proc) return;
    ...
    connect(m_proc, &QProcess::readyReadStandardOutput, this, &PortScope::onOut);
    connect(m_proc, &QProcess::readyReadStandardError, this, &PortScope::onErr);
    connect(m_proc, &QProcess::finished, this, &PortScope::onDone);
    m_proc->start(kBackend, args);
    m_btnRun->setEnabled(false); m_btnStop->setEnabled(true);
}

void PortScope::stopTool() {
    if (m_proc) { m_log->append("[stopping…]"); m_proc->kill(); }
}

void PortScope::onOut() { m_log->append(QString::fromLocal8Bit(m_proc->readAllStandardOutput()).trimmed()); }
void PortScope::onErr() { m_log->append(QString::fromLocal8Bit(m_proc->readAllStandardError()).trimmed()); }

void PortScope::onDone(int code) {
...
void PortScope::showAbout() {
    QMessageBox::about(this, "About PortScope",
        "PortScope v0.1.0 — © 2026 oday, All Rights Reserved (Proprietary).\n\n"
